...
export default function Poster({ poster, name, progressPercent = 0 }: Props) {
  ...
  return (
    <div className="relative flex-shrink-0 w-96 max-md:w-40 rounded-2xl overflow-hidden shadow-2xl shadow-black/80 border border-white/10">
      <img
        src={poster}
        alt={name}
        className="w-full aspect-[2/3] object-cover"
        onError={(e) => (e.currentTarget.src = '/icons.png')}
      />
      {hasProgress && (
