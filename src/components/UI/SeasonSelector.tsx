...
interface SeasonSelectorProps {
  ...
  isFocused?: boolean; // foco via controle remoto
  isDropdownOpen?: boolean; // abre dropdown via controle remoto (OK)
}
...
export const SeasonSelector = ({
  ...
  // Dropdown abre via clique manual OU via OK do controle remoto
  const isOpen = isOpenManual || isDropdownOpen;
  ...
  // Fechar dropdown manual quando o foco via controle remoto sai
  useEffect(() => {
    if (!isFocused) setIsOpenManual(false);
  }, [isFocused]);

  // Scroll para o item ativo no dropdown quando a season muda via controle remoto
  useEffect(() => {
  ...
  if (seasons.length <= 6) {
    return (
      <div
        className={`flex items-center gap-2 flex-wrap rounded-xl p-1 transition-all duration-200 ${
          isFocused ? 'ring-2 ring-red-500 ring-offset-2 ring-offset-zinc-950' : ''
        }`}
          ...
          return (
            ...
            >
              ...
              {hoveredSeason === season.number && (
                ...
                >
                  {watched}/{total} eps
                  {percent === 100 && <span className="text-green-400 ml-1">✓</span>}
                </div>
              )}
            </button>
          );
  ...
  const { watched, total, percent } = active
    ? getSeasonProgress(active)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 text-left">
          ...
          <div className="text-zinc-500 text-xs">
            {total} episódios
            {percent > 0 && <span className="text-green-400 ml-1.5">· {percent}% assistido</span>}
          </div>
        </div>
      ...
      {isOpen && (
        <div
          className="absolute top-full right-0 mt-2 w-full min-w-[300px]
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isCurrent && (
                      <span
                        className="text-[10px] bg-red-600/20 text-red-400
