...
export const ConfigServer = () => {
  ...
  const handleSelect = async (server: ServerEntry) => {
    ...
    setAuthenticated(true);
    await fetchServerContent(server); // Força atualização do conteúdo com o novo servidor
  };
  ...
  useRemoteControl(
    {
      ...
      onGreen: () => {
        if (!showForm && !inHeader && servers.length > 0) handleSelect(servers[focusedIndex]);
      },
      onYellow: () => {
        if (!showForm && !inHeader && servers.length > 0) handleEdit(servers[focusedIndex]);
      },
      onRed: () => {
      ...
      <div className="max-w-6xl mx-auto mt-[70px] h-[calc(100vh-60px)]">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-3">
            <ButtonBack onClick={handleBack} isFocused={focusedBack}>
              Voltar
            </ButtonBack>
            {!showForm && (
              <Button
